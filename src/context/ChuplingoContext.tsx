...
function fastShuffle<T>(list: T[]): T[] {
  const arr = [...list];
...
export const ChuplingoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchQuestionsFromSupabase = useCallback(async () => {
    ...
    try {
      // 1. Obtener conteo total exacto en Supabase
      ...
      // 3. Cargar lote representativo para administración y vistas previas
  ...
  const fetchQuestionsForSession = useCallback(async (params: {
    ...
    try {
      ...
      if (params.mode === 'simulacro') {
        // Para simulacro multidisciplinario: consultamos entre los 8 cursos
        const { data, error } = await supabase
          .from('questions')
          .select('*')
          .eq('active', true)
      ...
      // Traer una muestra amplia aleatoria para el lote
      ...
      // Fallback 1: Si fue muy estricto por tema o dificultad, buscar por curso completo
      if (params.courseId) {
      ...
      // Fallback 2: Banco general activo
    ...
    try {
      ...
      try {
        const { data: reqData } = await supabase
          ...
          .select('*')
          .eq<dyad-write path="src/context/ChuplingoContext.tsx" description="Completar la lógica de contexto de Chuplingo con soporte de conteo exacto de 8000 preguntas y Supabase">
"use client";
