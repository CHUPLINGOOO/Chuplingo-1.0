...
import { COURSES, INITIAL_QUESTIONS } from '../data/coursesData';
import { Question, QuestionAttempt, CourseId, PracticeMode } from '../types/chuplingo';
import { useChuplingo } from '../context/ChuplingoContext';
import { Star, ArrowRight, CheckCircle2, XCircle, Clock, Lightbulb, AlertTriangle, ArrowLeft } from 'lucide-react';
...
const PracticeQuestionScreen: React.FC = () => {
  ...
  const sessionQuestions = useMemo(() => {
    ...
    // If filter yields too few questions for demo, fill with course fallback questions
    if (list.length === 0) {
      list = allQuestions.filter(q => q.courseId === courseParam);
    }
    if (list.length === 0) {
      list = allQuestions; // Ultimate fallback
    }

    // Limit count based on mode
    const limit = modeParam === 'rapida' ? 10 : modeParam === 'estandar' ? 20 : modeParam === 'intensiva' ? 30 : list.length;
    return list.slice(0, Math.min(limit, list.length));
  }, [allQuestions, courseParam, topicParam, modeParam, mistakes, favoriteQuestionIds]);
  ...
  if (!currentQuestion) {
    return (
      <div className="p-6 text-center">
        <p className="text-sm font-bold text-slate-600">No se encontraron preguntas para los filtros seleccionados.</p>
        <button
    ...
    <div className="min-h-screen bg-[#F7F8FC] flex flex-col justify-between">
      ...
      <div className="bg-white px-4 pt-4 pb-3 border-b border-slate-100 shadow-sm sticky top-0 z-30">
        <div className="flex items-center justify-between mb-2">
          <button
            onClick={() => {
              if (window.confirm('¿Seguro que deseas salir de la práctica en curso?')) {
                navigate(-1);
              }
            }}
      ...
      <div className="p-4 flex-1 flex flex-col gap-4 max-w-[430px] mx-auto w-full">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {currentQuestion.universidad && (
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-200 text-slate-700">
              {currentQuestion.universidad} {currentQuestion.anio || ''}
            </span>
          )}
        </div>
        ...
        {isAnswerLocked && (
          <div className={`p-4 rounded-2xl border animate-in fade-in slide-in-from-bottom-2 duration-200 ${
            ...
            {currentQuestion.explicacion && (
              <div className="mt-2 text-xs leading-relaxed text-slate-700 bg-white/70 p-2.5 rounded-xl">
        ...
        <div className="max-w-[430px] mx-auto">
          {isAnswerLocked ? (
            <button
              ...
              style={{ backgroundColor: activeCourse.colorHex }}
            >
              <span>{isLastQuestion ? 'Finalizar Práctica 🎉' : 'Siguiente pregunta'}</span>
